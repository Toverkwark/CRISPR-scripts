...
echo Processing $1
OutputPrefix="/home/NKI/b.evers/bastiaan/gRNAs/CRISPR/kinome/qualityfiles"
if [ ! -f $OutputPrefix/$1.qualities.4 ]; then
	...
	perl ObtainValidProtospacersFromRefSeq.pl -r $1 -o $OutputPrefix/$1.protospacers
	...
	bowtie2 /home/NKI/b.evers/Genomes/hg19/genome-index/hg19 -r $OutputPrefix/$1.protospacers -t -S $OutputPrefix/$1.matched --no-hd --score-min L,-5,0 -k 2 --mm
	...
	perl IdentifyIdenticalTargetSites.pl -i $OutputPrefix/$1.matched -o $OutputPrefix/$1.qualities
	...
	perl QualityScoreSites.pl -i $OutputPrefix/$1.qualities -o $OutputPrefix/$1.qualities.0 -d 0 -s 12 
	...
	perl QualityScoreSites.pl -i $OutputPrefix/$1.qualities.0 -o $OutputPrefix/$1.qualities.1 -d 1 -s 20 -l
	echo 'Find 2MM relatives for' $1
	perl QualityScoreSites.pl -i $OutputPrefix/$1.qualities.1 -o $OutputPrefix/$1.qualities.2 -d 2 -s 20
	echo 'Find 3MM relatives for' $1
	perl QualityScoreSites.pl -i $OutputPrefix/$1.qualities.2 -o $OutputPrefix/$1.qualities.3 -d 3 -s 20
	echo 'Find 4MM relatives for' $1
	perl QualityScoreSites.pl -i $OutputPrefix/$1.qualities.3 -o $OutputPrefix/$1.qualities.4 -d 4 -s 20

	#Clean up intermediate files
	echo 'Clean up intermediate files for' $1
	rm $OutputPrefix/$1.protospacers
	rm $OutputPrefix/$1.matched
	rm $OutputPrefix/$1.qualities
	rm $OutputPrefix/$1.qualities.0
	rm $OutputPrefix/$1.qualities.1
	rm $OutputPrefix/$1.qualities.2
	rm $OutputPrefix/$1.qualities.3
fi
